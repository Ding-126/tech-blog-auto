# 面向面试之 Redis 系列七（番外）：缓存与数据库一致性——4 种方案从简单到可靠
...
| 场景 | 现象 | 发生频率 |
|------|------|---------|
| 先删缓存再更新 DB | 另一个线程读到旧数据写进缓存，彻底不一致 | 高 |
| 先更新 DB 再删缓存 | 删缓存失败，数据永久不一致 | 中 |
| 先更新 DB 再更新缓存 | 并发写时缓存被旧值覆盖 | 高 |
...
| 方案 | 一致性 | 复杂度 | 侵入性 | 适用场景 |
|------|--------|--------|--------|---------|
| Cache Aside | 弱一致（低并发下够用） | ⭐ 极低 | 低 | 大多数项目默认选择 |
| 延迟双删 | 最终一致（~500ms） | ⭐ 低 | 低 | 中小项目，能容忍短暂不一致 |
| MQ 最终一致 | 最终一致（~1s） | ⭐⭐⭐ 中 | 中 | 需要重试保障的场景 |
| Canal + Binlog | 最终一致（~1-3s） | ⭐⭐⭐⭐ 高 | **无** | 大型项目、高一致性要求 |

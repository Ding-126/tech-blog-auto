# Spring Boot 测试实战：Unit Test + Integration Test + Testcontainers
...
| 现象 | 原因 | 方案 |
|------|------|------|
| 测试启动慢（> 30秒） | Dcoker 镜像未缓存或为非静态容器 | 用 `static` 字段共享容器；先 `docker pull` 预拉取 |
| M1/M2 Mac 跑不起来 | ARM 架构不兼容 x86 镜像 | 加 `.withImagePlatform("linux/arm64")` 或用 ARM 原生镜像 |
| 随机端口冲突 | 多个测试类各自起了容器 | 用单例容器（Singleton Container 模式） |
| Testcontainers Desktop 提示付费 | 2025 年改成了商业授权 | CLI 模式不受限；CI 环境用 `ryuk` 容器清理 |
| 测试里拿不到容器 JDBC URL | `@DynamicPropertySource` 是静态方法，不能访问实例字段 | 容器声明为 `static`，属性源方法也设为 `static` |
...
| 指标 | 改造前（纯 H2 + Mock） | 改造后（Testcontainers） |
|------|---------------------|------------------------|
| 单次集成测试启动 | 2s（但 H2≠MySQL） | 12s（真实 MySQL） |
| 测试覆盖率 | 52% | 83% |
| 线上 SQL 问题 | 每季度 2-3 个 | 0（有 3 个被集成测试拦截） |
| CI 总时长 | 8min | 15min（但更可信） |
...
原文链接：https://testcontainers.com/guides/getting-started-with-testcontainers-for-java/

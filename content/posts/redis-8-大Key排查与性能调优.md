...
draft = false
title = '面向面试之 Redis 系列八（番外）：大 Key 排查与性能调优——线上实战记录'
description = '大 Key 阻塞、热 Key 打爆带宽、慢查询拖垮 Redis——本文从现象到排查到根治，用一个真实线上案例完整回溯排查过程。'
tags = ['Redis', '大Key', '热Key', 'Redis优化', '性能排查', '慢查询', 'Java后端', '实战']
categories = ['tutorial']
source_url = ''
source_name = 'original'
difficulty = '实战'
target_keywords = 'Redis 大Key, 热Key, Redis 性能优化, Redis 慢查询, bigkeys'

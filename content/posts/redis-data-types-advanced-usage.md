+++
date = '2026-06-17T07:10:00+08:00'
draft = false
title = '面向面试之 Redis 系列一：五种存储类型的高级用法，90% 的人只知道 String 和 List'
description = 'Redis 不只有 String 和 List，本文带你吃透五种数据类型的实战用法，包括 Hash 存对象、ZSet 做排行榜、Bitmap 签到、HyperLogLog 去重统计。面试高频题一次讲清楚，附完整代码。'
tags = ['Redis', '数据类型', '面试题', 'Java后端', 'Redis教程', '数据结构', '缓存']
categories = ['tutorial']
source_url = ''
source_name = 'original'
difficulty = '入门'
target_keywords = ['Redis 数据类型', '高级用法', 'Redis面试', '数据结构']
